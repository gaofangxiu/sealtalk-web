        ...
        if(mainDataServer.loginUser.token){
          RongIMSDKServer.connect(<string>mainDataServer.loginUser.token).then(function(userId) {
              ...
              RongIMLib.RongUploadLib.init(
                {domain:'http://7xogjk.com1.z0.glb.clouddn.com/',drop_element:'chatMain',container:'MessageForm',browse_button:'upload-image'},
                {domain:'http://o83059m7d.bkt.clouddn.com/',drop_element:'chatMain',container:'MessageForm',browse_button:'upload-file'}
              );
          }, function(error) {
              if (error.tokenError) {
                  //token 错误。
